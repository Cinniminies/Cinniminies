...
- [x] Auth, layout mobile-first con la paleta de 2.3 y PWA instalable. Login con link por mail o contraseña (en iPhone: la primera vez link en Safari, poner contraseña y entrar con ella en la app instalada; sin SMTP propio el mail no puede traer código). Detalle en `admin/README.md`.
...
- [x] `POST /api/pedidos` (`api/pedidos.js` + `api/_lib/pedidos.js`) llama a `crear_pedido_web(p, ip_hash)` (migración `20260930100000_pedidos_web.sql`):
  ...
  - anti-spam: campo trampa `sitio_web` (se responde 200 sin guardar), límite de 5 pedidos cada 15 min por IP (solo se guarda un hash) y tamaños máximos; además, un tope general de 40 pedidos por hora (trigger `privado.tope_pedidos_web`).
  - si se borra la venta de un pedido confirmado, el pedido vuelve a `nuevo` (trigger `privado.pedido_vuelve_a_nuevo`).
...
- [x] Sacado el POST a `GOOGLE_SHEETS_URL` del checkout (OK de los dueños, 25/09). Queda pendiente que **los dueños desactiven ese Apps Script** (Implementar → Administrar implementaciones → Archivar); la planilla "Pedidos Web (para importar)" puede quedar como archivo.
